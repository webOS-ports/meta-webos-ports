...
EXPORT_WEBOS_QMAKE_MACHINE += "${@ 'export MACHINE=${WEBOS_QMAKE_MACHINE}' if d.getVar('WEBOS_QMAKE_MACHINE', True) != '' and d.getVar('PACKAGE_ARCH', True) == d.getVar('MACHINE_ARCH', True) else '' }"
...
EXPORT_WEBOS_QMAKE_TARGET = "${@ 'MACHINE=${WEBOS_QMAKE_TARGET}' if d.getVar('WEBOS_QMAKE_TARGET', True) != '' and d.getVar('PACKAGE_ARCH', True) == d.getVar('MACHINE_ARCH', True) and not bb.data.inherits_class('native', d) else '' }"

# meta-oe's nodejs 24.19.0, renamed to nodejs24 so it does NOT become the
# default nodejs-native. Chromium 151 pins node v24 (third_party/node/
# update_node_binaries) and check_version.js compares the major version, but
# meta-oe on wrynose carries 22.23.1 and everything else here builds against
# that: enact-dev-native, webos_enactjs_{env,app}.bbclass, enyo-1.0,
# node-gyp-native, jsdoc-to-ts-native, localization-tool-native, upm, mraa.
# Adding 24.19.0 under its own name keeps them on 22 while webruntime gets 24.
# Only the native variant is built - nothing ships node on target.
#
# Upstream: meta-openembedded master, meta-oe/recipes-devtools/nodejs/.
# On upgrade, re-copy from there and re-apply this header plus the
# BBCLASSEXTEND change below, and re-convert the SPDX 'AND' in LICENSE to '&'
# (wrynose's oe-core predates SPDX expression support in license-format QA).
...
# target node would collide with meta-oe's nodejs; this fork is host-only

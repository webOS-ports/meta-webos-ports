...
# pulseaudio.service's ExecStart names the script directly now (-nF
# /etc/pulse/webos-system.pa), so the file had no remaining content or purpose.
...
# A machine that genuinely needs a different webos-system.pa (none currently do)
# can still override it the normal FILESEXTRAPATHS/${MACHINE} way.
SRC_URI = " \
...
# both hit exactly this. Confirmed 2026-09-11: not yet fixed on the actual
# device (module-null-source.so was not installed), just not yet exercised
# because module-droid-card itself is currently disabled there - see
# webos-system.pa.

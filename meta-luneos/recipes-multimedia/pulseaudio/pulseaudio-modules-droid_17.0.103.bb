...
# The plain git:// fetcher above unpacks into ${WORKDIR}/git, but OE-core's
# default S is ${WORKDIR}/${BP} (pulseaudio-modules-droid-17.0.103+git) - a
# directory the fetcher never populates. Without this, do_patch silently
# no-ops ("No file to patch. Skipping patch.") against that empty directory
# and do_configure then fails to find meson.build. Same bug shape as
# meta-luneos/recipes-webos-ose/wam/wam.inc before it gained the same line.
S = "${WORKDIR}/git"

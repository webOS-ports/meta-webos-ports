# Mirrored verbatim from NobuoTsukamoto/meta-tensorflow-lite @ main
#   recipes-framework/tensorflow-lite/libtensorflow-lite_2.21.0.bb (+ files/)
# That layer declares LAYERSERIES_COMPAT "wrynose whinlatter walnascar styhead"
# and LAYERDEPENDS "core openembedded-layer", so it could equally be added to
# bblayers.conf instead of mirrored - see faceunlock-research.md section 10.
#
# Why not webOS OSE's tensorflow-lite_2.9.3: libfart includes
# tensorflow/lite/delegates/nnapi/nnapi_delegate_c_api.h, which does not exist
# before ~2.10, so it cannot compile against 2.9.3 at all. 2.9.3 is also 2022
# code against a 2026 toolchain.
#
# Caveats carried over from upstream:
#  - do_configure[network] = "1": TFLite's CMake pulls several deps via
#    FetchContent at configure time, so this is not an offline build.
#  - XNNPACK is enabled on aarch64 only; 32-bit ARM falls back to RUY.
#  - KleidiAI (libkleidiai.so) is picked up on aarch64 when built.
#  - FINSANE_SKIP near the bottom is an upstream typo for INSANE_SKIP; left
#    as-is to keep this a verbatim mirror.

...
# configuration file for mp01 (Minimal Phone MP01, MediaTek Helio G99)
#
# Identical to halium-arm64.cmake on purpose. This machine builds the boot
# image, not a rootfs - the rootfs it runs is the generic halium-arm64 one - so
# every module split here has to match that machine's, or the packages would
# disagree with the image they end up in. It exists only because nyx-modules
# has "SRC_URI:append = file://${MACHINE}.cmake" and a machine without one does
# not parse.
#
# Nothing device-specific belongs here for the same reason it does not belong
# in halium-arm64.cmake: a device that does not match the compiled-in input or
# sysfs paths is the runtime device-config service's problem. In particular the
# MP01's physical keyboard and touchscreen are input nodes, not nyx modules -
# see 70-mp01.rules in meta-minimal.
#
# The E Ink panel is deliberately absent from this file too. Its refresh-mode
# control is an I2C sysfs attr group owned by a stock vendor module
# (panel-z10-eink-i2c.ko), which is a display concern rather than a nyx one,
# and nothing in LuneOS drives it yet.
